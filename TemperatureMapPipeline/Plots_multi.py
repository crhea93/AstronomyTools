...
class Bin:
    ...
    def add_temp(self,temp1,temp2):
        self.temp = temp1
        self.temp2 = temp2
    def add_stat(self,stat):
        self.stat = stat
    def add_abund(self, abund1, abund2):
        self.abund = abund1
        self.abund2 = abund2
...
def plot_Ab_multi(bin_file,temp_file,file_dir,filename,color_map,stn,wcs_image):
...
#       file_dir - Full Path to location of new image
#       fileame - name of file to be printed
def plot_Bins_multi(bin_file,temp_file,file_dir,filename,color_map,stn,wcs_image):
    #Get WCS information from header of original image
    hdu = fits.open(wcs_image)[0]
    wcs = WCS(hdu.header)
    Bins, x_min, x_max, y_min, y_max = read_in(bin_file,temp_file)
    create_image_fits(wcs_image,file_dir,x_min,x_max,y_min,y_max,Bins)
    fig = plt.figure()
    fig.set_size_inches(7, 7)
    ax = plt.axes(xlim=(x_min,x_max), ylim=(y_min,y_max),projection=wcs)
    #plt.axes(projection=wcs)
...
def read_in(bin_data,temp_data):
    ...
    for line in temp_d:
        bins[int(int(line.split(" ")[0]))].add_temp(float(line.split(" ")[1]),float(line.split(" ")[2]))
        bins[int(int(line.split(" ")[0]))].add_abund(float(line.split(" ")[3]),float(line.split(" ")[4]))
        bins[int(int(line.split(" ")[0]))].add_stat(float(line.split(" ")[5]))
    temp_d.close()
    bin_d.close()
...
#-------------------------------------------------#
#-------------------------------------------------#
def create_image_fits(fits_img,outroot,min_x,max_x,min_y,max_y,bins):
    # Create image array
    ...
    fits.writeto(outroot+'/temp_WVT.fits', temp_array.T, hdr, overwrite=True)
    hdu = fits.PrimaryHDU(abund_array)
    fits.writeto(outroot+'/abund_WVT.fits', abund_array.T, hdr, overwrite=True)
    ...
    for bin in bins:
        for pixel in bin.pixels:
            temp_array[int(pixel.pix_x-1),int(pixel.pix_y-1)] = bin.temp2
            abund_array[int(pixel.pix_x-1),int(pixel.pix_y-1)] = bin.abund2
    # Copy header
    fits_ = fits.open(fits_img)
    hdr = header=fits_[0].header
    # Change image
    hdu = fits.PrimaryHDU(temp_array)
    hdul = fits.HDUList([hdu])
    fits.writeto(outroot+'/temp2_WVT.fits', temp_array.T, hdr, overwrite=True)
    hdu = fits.PrimaryHDU(abund_array)
    fits.writeto(outroot+'/abund2_WVT.fits', abund_array.T, hdr, overwrite=True)
