...
def specextract_run(obsid,filenames,file_to_convert,outfile_from_convert,output_dir):
    ...
    if background.lower() == 'blank': #using blank-sky background
    ...
    specextract.asp = filenames['asol1']
    specextract.mskfile = filenames['msk1']
...
def create_reg_comb(pix_in_bin,file_to_split,output_dir):
    points = {} #(x,[y]) pairs
...
def split_fits(obsid,filenames,file_to_split,output_file,output_dir,pix_in_bin_num,bin_number):
    ...
    create_reg_comb(pix_in_bin_num,file_to_split,output_dir)
...
def create_spectra(filename,OBSIDS,source_file,output_dir):
    for obsid in OBSIDS:
        print("We are on obsid %s"%obsid)
        directory = base_directory+'/'+obsid
        ...
        for bin_i in bins:#bin_unique: #for each unique_bin
            ...
            spec_to_combine = []
            print("We are combining bin number "+str(bin_i.bin_number)+" of "+str(len(bins)))
...
def main():
    create_spectra(filename,OBSIDS,source_file,output_dir)
main()
